...
object Fixtures:
  ...
  val penManufacturer =
    Optic.thatCan.editOption[Pen, Company](_.manufacturer)(m => p => p.copy(manufacturer = p.manufacturer.map(_ => m)))
  ...
  val deskPrinter =
    Optic.thatCan.editOption[Desk, Printer](_.printer)(p => desk => desk.copy(printer = desk.printer.map(_ => p)))
  ...
  val office =
    Office(Desk(4, Some(Printer(true))), List(Pen("red", None), Pen("green", Some(Company("BobCorp"))), Pen("blue")))
