...
private[monocle] trait LensImpl[+Can <: Get & Modify, -Structure, +Modified, +Out, -In]
    extends OptionalImpl[Can, Structure, Modified, Out, In]
    with NonEmptyTraversalImpl[Can, Structure, Modified, Out, In]
    with GetterImpl[Can, Structure, Modified, Out, In]:
  ...
  override protected[impl] def nonEmptyModifyA[F[+_]: Apply](f: Out => F[In])(s: Structure): F[Modified] =
    modifyF(f)(s)
  ...
  protected def composeLens[Can2 >: Can <: Get & Modify, Out2, In2](
    optic2: LensImpl[Can2, Out, In, Out2, In2]
