...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(Mono.optI.andThen(Prism.some).getOption(monoNone))


    val poly1 = Poly(1, FooIS(8, "hey"), Some(0), Option(3))
    // change a Poly[Option[Int]] to  Poly[Option[String]]
    println(Poly.pparam.andThen(PPrism.some).replace("hello")(poly1))
...
//    println(Poly.pparam.andThen(PPrism.some).replace("hello")(poly2))
    ...
    println(Poly.param.andThen(Prism.some).andThen(Foo.fooI).replace(FooI(7))(poly3))
...
//    println((Poly.param.andThen(PPrism.some.andThen(Foo.fooIS).getOrError(poly3.copy(param = None)))
    ...
    // List(1, 3, 6, 10)
    println(PTraversal.pair.andThen(PTraversal.pair).toList(((1, 3), (6, 10))))
    // List(1, 3, 6, 10)
    println(PTraversal.pair.andThen(PTraversal.pair).toList(((1, 3), (6, 10))))

    // Left(Expected FooI but got FooS(hello))
    println(PTraversal.list.andThen(Foo.fooI).toList(List(FooI(10), FooS("hello"), FooI(0), FooIS(3, "world"))))
    // Left(List is empty)x
    println(PTraversal.list.andThen(Foo.fooI).toList(Nil))
    // Right(List(FooI(10), FooI(0)))
//    println((PTraversal.list >>>? Foo.fooI).toListOrError(List(FooI(10), FooS("hello"), FooI(0), FooIS(3, "world"))))

    // Right(List(1, 2, 3, 4))
    println(Foo.fooL.andThen(PTraversal.list).toList(FooList(List(1,2,3,4))))
    // Left(List is empty)
    println(Foo.fooL.andThen(PTraversal.list).toList(FooList(Nil)))
    // Left(Expected FooList but got FooI(10))
    println(Foo.fooL.andThen(PTraversal.list).toList(FooI(10)))
